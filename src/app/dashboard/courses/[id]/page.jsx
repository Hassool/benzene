      ...
      <div className="bg-bg-secondary dark:bg-bg-dark-secondary border border-border dark:border-border-dark rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className={`flex items-center space-x-3 mb-4 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
          ...
          <h3 className="text-lg font-montserrat font-semibold text-text dark:text-text-dark">
            {t("coursePage.deleteModal.title")}
          </h3>
        </div>
        
        <p className={`text-text-secondary dark:text-text-dark-secondary font-inter mb-4 leading-relaxed ${isRTL ? 'text-right' : ''}`}>
          {t("coursePage.deleteModal.confirmMessage")} <span className="font-semibold text-text dark:text-text-dark">"{section?.title}"</span>? 
        </p>
        
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-6">
          <p className={`text-red-800 dark:text-red-200 text-sm font-inter ${isRTL ? 'text-right' : ''}`}>
            <strong>{t("coursePage.deleteModal.warningTitle")}</strong> {t("coursePage.deleteModal.warningMessage")}
          </p>
        </div>
        ...
        <div className={`flex items-center justify-end space-x-3 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
          ...
          >
            {t("coursePage.deleteModal.cancelButton")}
          </button>
          <button
            onClick={onConfirm}
            disabled={isDeleting}
            className={`px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors font-inter font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}
          >
            {isDeleting ? (
              <>
                ...
                <span>{t("coursePage.deleteModal.deletingButton")}</span>
              </>
            ) : (
              t("coursePage.deleteModal.deleteButton")
            )}
          </button>
        </div>
      </div>
              ...
              <div className="bg-bg-secondary dark:bg-bg-dark-secondary border border-border dark:border-border-dark rounded-xl p-6">
                <div className={`flex items-center justify-between mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <div className={isRTL ? 'text-right' : ''}>
                    <h2 className="font-montserrat font-semibold text-xl text-text dark:text-text-dark">
                      {t("coursePage.sections.title")}
                    </h2>
                    <p className="text-sm text-text-secondary dark:text-text-dark-secondary font-inter mt-1">
                      {courseSections.length} {courseSections.length === 1 ? t("coursePage.sections.totalSection") : t("coursePage.sections.totalSections")} {t("coursePage.sections.total")}
                    </p>
                  </div>
                  <Link href={`addContent/${id}`} className={`inline-flex items-center px-4 py-2 bg-special hover:bg-special-hover dark:bg-special-dark dark:hover:bg-special text-white rounded-lg transition-colors font-inter font-medium ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <svg className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                    {t("coursePage.sections.addSectionButton")}
                  </Link>
                </div>
                ...
                {sectionError && (
                  <div className="mb-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <p className={`text-yellow-800 dark:text-yellow-200 font-inter text-sm ${isRTL ? 'text-right' : ''}`}>
                      {t("coursePage.sections.errorMessage")} {sectionError}
                    </p>
                  </div>
                )}
                  ...
                  {courseSections.length === 0 ? (
                    <div className="text-center py-12 bg-bg dark:bg-bg-dark rounded-lg border border-border dark:border-border-dark">
                      ...
                      <h3 className="font-montserrat font-semibold text-text dark:text-text-dark mb-2">
                        {t("coursePage.sections.noSectionsTitle")}
                      </h3>
                      <p className="text-text-secondary dark:text-text-dark-secondary font-inter mb-4">
                        {t("coursePage.sections.noSectionsMessage")}
                      </p>
                      <Link href={`addContent/${id}`} className={`inline-flex items-center px-4 py-2 bg-special hover:bg-special-hover dark:bg-special-dark dark:hover:bg-special text-white rounded-lg transition-colors font-inter font-medium ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <svg className={`w-4 h-4 ${isRTL ? 'ml-2' : 'mr-2'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                        {t("coursePage.sections.createFirstSectionButton")}
                      </Link>
                    </div>
                  ) : (
                    courseSections.map((section, index) => (
                      <div key={section._id} className="group bg-bg dark:bg-bg-dark rounded-lg border border-border dark:border-border-dark hover:border-special/50 dark:hover:border-special-dark/50 transition-all duration-200 overflow-hidden">
                        ...
                        <div className={`flex items-center justify-between p-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                          <div className={`flex items-center space-x-4 flex-1 min-w-0 ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
                            ...
                            <div className={`flex-1 min-w-0 ${isRTL ? 'text-right' : ''}`}>
                              ...
                              <div className={`flex items-center gap-4 text-sm text-text-secondary dark:text-text-dark-secondary ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                {section.isPublished !== undefined && (
                                  <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${
                                    ...
                                    {section.isPublished ? t("coursePage.sections.published") : t("coursePage.sections.draft")}
                                  </span>
                                )}
                              </div>
                            </div>
                          ...
                          {/* Action Buttons */}
                          <div className={`flex items-center space-x-2 opacity-75 group-hover:opacity-100 transition-opacity ${isRTL ? 'flex-row-reverse space-x-reverse' : ''}`}>
                            <button 
                              ...
                              title={t("coursePage.sections.editTooltip")}
                            >
                            ...
                            <Link href={`/Courses/${courseId}/${section._id}`} 
                              ...
                              title={t("coursePage.sections.previewTooltip")}
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            ...
                            <button 
                              ...
                              title={t("coursePage.sections.deleteTooltip")}
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              </svg>
                        ...
                        {section.description && (
                          <div className="px-4 pb-4">
                            <p className={`text-text-secondary dark:text-text-dark-secondary font-inter text-sm leading-relaxed ${isRTL ? 'pr-14 text-right' : 'pl-14'}`}>
                              {section.description}
                            </p>
                          </div>
                        )}
