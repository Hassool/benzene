// src/components/EditCourse.jsx - Edit Course Component
"use client"

import { useState, useEffect } from "react"
import { useSession } from "next-auth/react"
import { Upload, Book, Image as ImageIcon, X, Check, Save } from "lucide-react"
...
export default function EditCourse({ courseId }) {
    ...
    const fetchCourse = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setErrors({ fetch: result.message || 'Erreur lors du chargement du cours' })
        }
      } catch (error) {
        setErrors({ fetch: 'Erreur de connexion lors du chargement du cours' })
      } finally {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (formData.thumbnail && formData.thumbnail !== originalThumbnail) {
        await deleteImageFromCloudinary(formData.thumbnail)
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (originalThumbnail && formData.thumbnail !== originalThumbnail) {
        await deleteImageFromCloudinary(originalThumbnail)
      }
      ...
      if (result.success) {
        setSuccess(true)
        setOriginalThumbnail(formData.thumbnail) // Update original thumbnail
        setErrors({})
      ...
      <div className="min-h-screen bg-gradient-light dark:bg-gradient-dark flex items-center justify-center">
        <div className="text-center p-8">
          ...
          <p className="text-text dark:text-text-dark">
            {status === 'loading' ? 'Chargement...' : 'Chargement du cours...'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="min-h-screen bg-gradient-light dark:bg-gradient-dark flex items-center justify-center">
        <div className="text-center p-8 bg-bg-secondary/50 dark:bg-bg-dark-secondary/50 backdrop-blur-sm rounded-2xl border border-border/30 dark:border-border-dark/30">
          ...
          <p className="text-text-secondary dark:text-text-dark-secondary mb-4">
            Veuillez vous connecter pour modifier ce cours.
          </p>
        </div>
      </div>
    )
  ...
  if (errors.fetch) {
    return (
      <div className="min-h-screen bg-gradient-light dark:bg-gradient-dark flex items-center justify-center">
        <div className="text-center p-8 bg-bg-secondary/50 dark:bg-bg-dark-secondary/50 backdrop-blur-sm rounded-2xl border border-border/30 dark:border-border-dark/30">
          <Book className="w-16 h-16 mx-auto text-red-500/70 mb-4" />
          <h3 className="text-xl font-semibold text-text dark:text-text-dark mb-2">
